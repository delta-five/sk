...
func TestMakePtrResult(t *testing.T) {
	...
	t.Run("with error", func(t *testing.T) {
		...
		r := sk.MakePtrResult[int](assert.AnError)
		val, err := r.Unwrap()
		require.Nil(t, val)
		assert.ErrorIs(t, err, assert.AnError)
	})
...
func TestResultMap(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "success",
			r:      sk.MakeResult(21, nil),
			mapper: func(n int) string { return strconv.Itoa(n * 2) },
			want:   "42",
		},
		{
			name:    "error",
			r:       sk.MakeResult(21, assert.AnError),
...
func TestResultMapSlice(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:    "error",
			r:       sk.MakeResult([]int(nil), assert.AnError),
			mapper:  func(n int) string { return "should not run" },
			wantErr: assert.AnError,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := tt.r.Map(sk.NewSliceMapper(tt.mapper))
			val, err := got.Unwrap()
			if tt.wantErr != nil {
				require.ErrorIs(t, err, tt.wantErr)
				assert.Empty(t, val)
...
func TestResultMapSliceWithError(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := tt.r.MapWithError(sk.NewSliceMapperWithError(tt.mapper))
			val, err := got.Unwrap()
			if tt.wantErr != nil {
				require.ErrorIs(t, err, tt.wantErr)
				assert.Empty(t, val)
...
func TestResultMapSliceWithErrors(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "success",
			r:      sk.MakeResult(slices.Sorted(maps.Keys(errMap)), nil),
			mapper: func(n int) (string, error) { return strconv.Itoa(n), nil },
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			got := tt.r.MapWithError(sk.NewSliceMapperWithErrors(tt.mapper))
			val, err := got.Unwrap()
			if tt.wantErr != nil {
